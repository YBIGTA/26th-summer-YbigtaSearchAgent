...
의미적 검색과 키워드 검색을 결합하여 최적의 검색 결과를 제공합니다.
...
class HybridRetriever:
    ...
    async def search(self, 
        ...
        try:
            ...
            if search_type == "semantic" and self.semantic_engine:
                ...
                results = await self._hybrid_search(processed_query, filters, top_k, sources)
    ...
    async def _hybrid_search(self, query: str, filters: Dict, top_k: int, sources: List[str]) -> Dict[str, Any]:
        """하이브리드 검색 실행 - RRF 융합"""
        ...
        try:
            ...
            # RRF로 결과 융합
            fused_results = await self._reciprocal_rank_fusion(search_results, top_k)
            
            return fused_results
            
        except Exception as e:
            logger.error(f"하이브리드 검색 실행 오류: {str(e)}")
            return {"documents": [], "scores": [], "metadata": []}
    
    async def _reciprocal_rank_fusion(self, search_results: Dict[str, Dict], top_k: int) -> Dict[str, Any]:
