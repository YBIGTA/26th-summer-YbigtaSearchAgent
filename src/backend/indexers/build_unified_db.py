...
# === 공통 설정 ===
load_dotenv()
CHROMA_DB_PATH = "data/unified_chroma_db/unified_chroma_db"
...
def run_notion(collection_name: str) -> int:
    print("=" * 50)
    print("Notion에서 데이터를 수집합니다...")
    
    # 캐시 파일 로드
    notion_cache_path = "notion_pages_cache.json"
    if os.path.exists(notion_cache_path):
        with open(notion_cache_path, "r", encoding="utf-8") as f:
            processed_pages = set(json.load(f))
        print(f"✅ 기존 캐시에서 {len(processed_pages)}개의 처리된 페이지 목록을 불러왔습니다.")
    else:
        processed_pages = set()
    ...
    # 캐시되지 않은 페이지만 필터링
    new_page_ids = [page_id for page_id in page_ids if page_id not in processed_pages]
    
    if not new_page_ids:
        print("⚡️ 캐시 히트! 모든 Notion 페이지가 이미 처리되었습니다.")
        return 0
    
    print(f"🔍 총 {len(page_ids)}개 페이지 중 {len(new_page_ids)}개 새 페이지 처리 예정")
    
    # 새 페이지들만 로드
    docs = asyncio.run(load_all_notion_pages(new_page_ids, api_key))
    ...
    if docs:
        ...
        # 캐시 업데이트
        processed_pages.update(new_page_ids)
